...
func cmdAddEval() {
	...
	if runGitCmd(cwd, "rev-parse", "HEAD~1") == "" {
	...
	diffStat := runGitCmd(cwd, diffArgs...)
	nameStatus := runGitCmd(cwd, nameArgs...)
	shortstat := runGitCmd(cwd, shortArgs...)
	log := runGitCmd(cwd, "log", "--oneline", "-3")
	diffContent := runGitCmd(cwd, "diff", "HEAD~1")

	if strings.TrimSpace(diffStat) == "" {
		fmt.Println("No file changes detected since last session.")
		fmt.Println("What to do: make a commit first, then run 'brain add --eval'.")
		return
	}
	...
	content := fmt.Sprintf("# Session — %s\n\n## Git Summary\n- Files created: %s\n- Files modified: %s\n- Files deleted: %s\n- Total changes: %s\n\n## Recent Commits\n%s\n\n---\n<!-- Agent: append your evaluation below this line -->\n",
		...
		formatListEval(created),
		formatListEval(modified),
		formatListEval(deleted),
	...
	topic := detectTopicFromDiffEval(diffContent)
...
func formatListEval(items []string) string {
	if len(items) == 0 {
		return "none"
	}
...
func detectTopicFromDiffEval(diff string) string {
