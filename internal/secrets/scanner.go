...
var Patterns = []struct {
	...
	{"Heroku API Key", regexp.MustCompile(`(?i)heroku_[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}`)},
	{"Twilio API Key", regexp.MustCompile(`SK[0-9a-fA-F]{32}`)},
	{"SendGrid API Key", regexp.MustCompile(`SG\.[A-Za-z0-9_-]{22}\.[A-Za-z0-9_-]{43}`)},
	{"Firebase Key", regexp.MustCompile(`(?i)firebase[_-]?(api[_-]?key|secret)\s*[:=]\s*["']?[A-Za-z0-9\-_.]{20,}["']?`)},
	{"NPM Token", regexp.MustCompile(`(?i)//registry\.npmjs\.org/:_authToken=[0-9a-f-]{36}`)},
