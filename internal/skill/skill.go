...
func WriteAdaptations(cwd string, content string) (int, error) {
	if strings.TrimSpace(content) == "" {
		return 0, nil
	}

	adaptationBlock := "\n## Project Adaptations\n\n" +
		AdaptationStartMarker + "\n" +
	...
	for _, dir := range dirs {
		...
		start := strings.Index(existing, AdaptationStartMarker)
		if start != -1 {
			end := strings.Index(existing, AdaptationEndMarker)
			if end != -1 {
				existing = existing[:start] + adaptationBlock + existing[end+len(AdaptationEndMarker):]
			}
		} else {
			existing = existing + "\n" + adaptationBlock
		}
