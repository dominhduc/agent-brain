...
### Fixed
- **`brain search` alias restored** — `brain search "query"` works again instead of showing "Unknown command". Redirects to `brain get` search mode.
- **`brain trace extract --dry-run` wording** — Now says "Would extract N entries" instead of misleading "Extracted N entries" in dry-run mode.
- **`brain grade` progress output** — Shows batch progress ("Grading batch 1/N...") so the user knows it's working during slow LLM calls.
...
- `brain update` auto-restarts the daemon if it was running before the update

### Changed
- `brain help` header uses `brain` (the CLI name) instead of `agent-brain` (the package name)
